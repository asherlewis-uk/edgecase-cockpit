...
> **Account requirement:** Saving a provider API key calls `POST /api/keys/set`, which stores the key encrypted in D1 and requires an authenticated `user_id`. If you are not signed in, `ProviderCard` shows a "Sign in to save provider keys securely" prompt and the Save button does not call the API. To complete this section, either create an account via the **Account** menu / `/auth` route first, or test the guest path and confirm the auth prompt appears.
>
> create account → return to Settings → enter key → save → status becomes Ready.
...
| 3.4  | Click the **Save** button.                                                                                            | If signed in: a success toast appears and the key is saved. If signed out: an inline auth prompt appears and no API call is made. | Signed in: toast says **"API key saved"** / **"Provider ready"**. Signed out: prompt says **"Sign in to save provider keys securely"**. | Signed in: no toast or generic error. Signed out: prompt missing or Save still calls the API. |
