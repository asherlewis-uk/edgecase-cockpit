...
| --- | --- | --- |
| Is there actual user signup today? | **Backend only.** The API endpoints exist, but there is no user-facing signup or login UI. | `src/routes/api/auth/register.ts`, `src/routes/api/auth/login.ts` exist. `src/routes/__root.tsx`, `src/routes/index.tsx`, `src/routes/settings.tsx` contain no login/register buttons, forms, or `/auth` route. |
| Is there email/password signup? | **API supports it; UI does not.** `POST /api/auth/register` accepts `email`, `password` (min 8 chars), and optional `displayName`. | `src/routes/api/auth/register.ts` |
...
| Is the current session anonymous, account-backed, or hybrid? | **Hybrid at the API layer, anonymous in practice.** Every visitor gets an encrypted cookie with a `session.id` and `guestSessionId`. The app does not expose any way to attach a `userId` to that cookie through the UI. | `src/lib/session.server.ts` (`getCockpitSession`, `getGuestSessionId`), `src/routes/api/auth/login.ts`, `src/routes/api/auth/register.ts` |
...
- A person opening the production URL lands as a guest.
- They cannot create an account, log in, or convert their guest session to an account through the app.
- The backend is fully ready to do all of that; the missing layer is the frontend.
...
| --- | --- | --- | --- |
| Provider API keys | Authenticated `user_id` in D1, encrypted | `user_provider_keys` PK is `(user_id, provider_id)`. `setProviderCreds` rejects guests. `/api/keys/status` only lists keys for the current `userId`. Cross-user tests exist. | No UI to become authenticated, so in practice keys cannot be stored for a normal visitor. |
...
| Settings | Authenticated `user_id` | `GET/POST /api/settings` return 401 for guests; `user_settings` table is `user_id` PK. | No UI to set an authenticated user, so server-side settings are unreachable in practice. |
...
- `/api/keys/set` — calls `setProviderCreds` which throws for guests (handler does not catch it, so guest requests fail).
...
- Actual UI login → save key → chat → thread sync across two real browser sessions.
- Cross-user data denial with real cookies (unit tests mock sessions).
...
| Are settings stored locally, remotely, or both? | Both paths exist, but only the local path is reachable today. `cockpit-store.ts` writes to `localStorage`; `/api/settings` writes to D1 for authenticated users. | `src/lib/cockpit-store.ts`, `src/routes/api/settings.ts` |
...
| What happens on a new browser/device? | A new guest cookie/session is created. `localStorage` threads/settings do not transfer. Server-side data is empty because the visitor is unauthenticated. | `src/lib/session.server.ts` `getGuestSessionId`, `src/lib/cockpit-store.ts` defaults. |
...
- **Gap:** No account prompt; every visitor is a guest.
...
- **Backend:** Onboarding state is purely local today (`localStorage`). Server-side `user_settings.onboarding_completed` exists but is unreachable without auth UI.
...
- **Current edge:** For an unauthenticated guest (the only state reachable through the UI), `setProviderCreds` throws. The handler does not catch this, so the UI save fails. This is the single largest functional gap in the current E2E flow.
- **Evidence:** `src/routes/api/keys/set.ts`, `src/lib/session.server.ts` `setProviderCreds`.

### 6. Provider key is saved/encrypted

- **User-visible:** Save succeeds only if the user is authenticated. There is currently no way to become authenticated in the UI.
...
- **Opt-in server sync:** If an authenticated user enables sync, `POST /api/threads` writes to D1. This path exists but is unreachable through the current UI.
...
- **Server:** `usage_records` and `provider_stats` are written for authenticated users. Currently unreachable.
...
- **User-visible:** Onboarding does not reappear; previous threads/settings are loaded from `localStorage`.
- **Backend:** Same encrypted cookie is recognized if still valid (30-day maxAge). Server-side data is still unreachable without auth.
- **Evidence:** `src/lib/session.server.ts`, `src/lib/cockpit-store.ts`.

### 18. New browser/device behavior

- **User-visible:** Fresh guest session; no threads, no settings, no keys.
- **Backend:** New cookie issued; D1 has no rows for this unauthenticated session.
...
- **Implemented and reachable:** Guest chat, `localStorage` threads/settings, offline queue, onboarding, provider selection, built-in tool execution, streaming responses, export/import.
- **Implemented but unreachable:** User accounts, encrypted provider key storage, server-synced threads, server-side settings, usage records, cross-device sync.
- **Not implemented:** Google Sign-In, Apple Sign-In, any OAuth, password reset, email verification.
- **Most important gap:** There is no UI to log in or sign up, so all server-side authenticated features are currently inaccessible to end users.
